...
class EditarProducto : AppCompatActivity() {
    ...
    fun initVariables(){
       ...
       /* if (nameProducto != null) {
        ...
        if (nameProducto != null){
            fireData.collection("db1").document(database).collection("Productos").document(nameProducto).addSnapshotListener { snapshot, error ->
                if (error != null) {
                    return@addSnapshotListener
                }
                if (snapshot != null){
                    println("Esta es: " + snapshot.data?.get("precio"))
                    editarPrecio_producto.text = Editable.Factory.getInstance().newEditable(snapshot.data?.get("precio").toString())
                    editarCantidadProducto.text = Editable.Factory.getInstance().newEditable(snapshot.data?.get("cantidad").toString())
                }
